...
**Goal:** Automated scenario testing that validates all 18 tools work across 20 eval scenarios, with pass/fail reporting and CI integration.

**Architecture:** Single script (`scripts/run_evals.py`) that loads scenarios, executes each scenario's expected tools with realistic params, chains context between tools within a scenario, and reports a pass/fail table. Uses the existing `api_server.create_api()` backend directly.

**Tech Stack:** Python stdlib only (json, time, sys, pathlib). No new dependencies.

---

### Task 1: Create the eval runner script
...
def params_get_claim_detail(api, ctx):
    ...
    if not claim_id:
        # Grab any claim for this member
...
def run_scenario(api, scenario, verbose=False):
    ...
    for tool_name in scenario["expected_tools"]:
        ...
        # Some errors are data-level (e.g. "No members found") — still a valid response
        # Only fail on unknown-tool or parameter errors
...
def main():
    ...
    # Initialize API (force JSON backend for consistent eval)
...
Run: `cd /Users/matthewjohnson/healthcare-synthetic-data && python3 scripts/run_evals.py`
Expected: All 20 scenarios pass with a table like:
```
EVAL-001  [PASS]  Member looks up their deductible status     (2 tools, 12ms)
...
20/20 passed | 0 failed | Total: NNNms
```

**Step 3: Test verbose mode**

Run: `python3 scripts/run_evals.py --verbose`
Expected: Same results but with per-tool detail lines.

**Step 4: Test difficulty filter**

Run: `python3 scripts/run_evals.py --filter Hard`
Expected: Only Hard scenarios run (EVAL-005, EVAL-012, EVAL-016, EVAL-020).

**Step 5: Commit**

```bash
git add scripts/run_evals.py
git commit -m "feat: add eval runner for automated scenario testing"
```

---

### Task 2: Add eval runner to CI

**Files:**
- Modify: `.github/workflows/ci.yml`

**Step 1: Add py_compile for new files**

In the "Compile Python files" step, add these lines to the list:
```
agent-sandbox/mcp_remote.py \
scripts/run_evals.py \
api/index.py
```

**Step 2: Add eval runner step**

After the "Validate generated data" step, add:
...
Run: `python3 -c "import yaml" 2>/dev/null || echo 'no yaml module, check manually'`
Then visually verify the YAML is correct.

**Step 4: Commit**

```bash
git add .github/workflows/ci.yml
git commit -m "ci: add eval runner and new file syntax checks"
```
...
Run: `python3 -m py_compile scripts/run_evals.py && python3 -m py_compile api/index.py && python3 -m py_compile agent-sandbox/mcp_remote.py && echo "All OK"`
Expected: "All OK"
